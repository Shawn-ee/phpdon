...
class IndexDynamicList extends ApiRest
{
    ...
    public function dynamicInfo(){
        ...
        //技师情况
        $data['coach_info'] = $this->model->where(['id'=>$data['coach_id']])->field('coach_name,work_img,is_work,work_time,status')->find();
    ...
    public function getFollowData(){
        ...
        $arr['num'] = $follow_model->getFollowDynamicNum($this->getUserId());
    ...
    public function followCoachList(){
        ...
        $coach_id = $shield_model->where(['user_id'=>$this->getUserId(),'type'=>2])->column('coach_id');
